...
const FriendCard = ({ friend }) => {
  ...
  return (
    <div onClick={() => navigate(`/friend/${friend.id}`)} className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 hover:shadow-xl cursor-pointer mb-2 mr-2">
     ...
     <div className={`${colors[friend.status]} text-white text-xs w-24 py-2 rounded-full font-bold uppercase`}>
    <h2>{friend.status}</h2>
  </div>
  </div>
    </div>
